...
/*
	getFolderName "f:/a/b/c" -> "c"
 */

FileSystem.getFolderName = function(file)
{
	if (file !== undefined)
	{
		if (file instanceof File)
		...
		if (a > b && a === file.length - 1)
		{
			file = file.substring(0, file.length - 1);
			a = file.lastIndexOf("\\");
			return file.substring(a + 1);
		}
		else if (a < b && b === file.length - 1)
		{
			file = file.substring(0, file.length - 1);
			b = file.lastIndexOf("/");
			return file.substring(b + 1);
		}
		return file.substring(a > b ? a + 1 : b + 1);
